...
REM Setup GnuCOBOL environment for project
echo %BLUE%[BUILD]%NC% Configuring GnuCOBOL environment...
for /f "tokens=*" %%i in ('where cobc') do set COBOL_BIN=%%i
for %%i in ("%COBOL_BIN%") do set COBOL_ROOT=%%~dpi..
set COB_CONFIG_DIR=%COBOL_ROOT%\share\gnucobol\config
set COB_COPY_DIR=%COBOL_ROOT%\share\gnucobol\copy
set COB_LIBRARY_PATH=%COBOL_ROOT%\lib

REM Create local config if system config unavailable
if not exist "%COB_CONFIG_DIR%\default.conf" (
    echo %YELLOW%[CONFIG]%NC% Creating local GnuCOBOL configuration...
    if not exist config mkdir config
    echo # CBLPolyCall GnuCOBOL Configuration > config\default.conf
    echo standard-define 1 >> config\default.conf
...
REM Build executable with environment configuration
